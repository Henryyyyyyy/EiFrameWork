package me.zengjin.base_framework.widge;
...
public class StatusBarUtil {
    ...
    private static void addTranslucentView(Activity activity, int statusBarAlpha) {
        ViewGroup contentView = (ViewGroup) activity.findViewById(android.R.id.content);
        Log.e("miaoaa","count="+contentView.getChildCount());

        if (contentView.getChildCount() > 1) {
            Log.e("miaoaa","c222222");
    ...
    public static boolean androidStatusBarLightMode(Activity activity, boolean isLiveHome) {
        ...
        if (SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (!flyMe && !MIUI) {
                if (SDK_INT >= Build.VERSION_CODES.KITKAT) { //4.4以上可以设置透明
                    StatusBarUtil.setTransparentForImageView(activity, null);
                    return isLiveHome;
                } else {
    ...
    public static void setWhiteStatusBar(Activity activity) {
        if(SDK_INT >= 21) {
            ...
            int count = decorView.getChildCount();
            if(count > 0 && decorView.getChildAt(count - 1) instanceof StatusBarView) {
                decorView.getChildAt(count - 1).setBackgroundDrawable(ContextCompat.getDrawable(activity.getApplicationContext(), R.drawable.shadow_status_bar));
            } else {
